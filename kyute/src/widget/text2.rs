...
impl Text2 {
    ...
    #[composable]
    pub fn new(formatted_text: impl Into<FormattedText>) -> Text2 {
    ...
    fn draw_glyph_run(&mut self, glyph_run: &GlyphRun, drawing_effects: &GlyphRunDrawingEffects) {
        ...
        if let Some(mask) = analysis.rasterize(raster_opts) {
            ...
            let mask_blender = {
                ...
                /*eprintln!(
                    "mask_blender: uniform_size={} offset={} u_size={}",
                    uniform_size, u_offset, u_size
                );*/
...
impl Widget for Text2 {
    ...
    fn paint(&self, ctx: &mut PaintCtx, _bounds: Rect, _env: &Environment) {
        ...
        /*for run in runs.as_ref().unwrap().iter() {
            let x = run.bounds.origin.x as f32 * ctx.scale_factor as f32;
            let y = run.bounds.origin.y as f32 * ctx.scale_factor as f32;
        }*/
    }
}
